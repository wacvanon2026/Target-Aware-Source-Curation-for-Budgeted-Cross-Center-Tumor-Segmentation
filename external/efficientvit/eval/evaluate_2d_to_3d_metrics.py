#!/usr/bin/env python3
"""Evaluate BraTS 2D-slice EfficientViT checkpoints as 3D cases.

This evaluator keeps the same volume reconstruction logic as
eval/evaluate_2d_to_3d.py, but writes per-case metrics and run metadata for
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Directory that will receive per_case_metrics.csv, summary_metrics.json, and run_metadata.json.",
    )
...
def infer_target_domain(*parts: str) -> str | None:
    ...
    for name in ("TCGA_LGG", "TCGA_GBM", "C4", "C5", "UPENN", "IVYGAP", "TCGA"):
        if re.search(rf"(^|[^A-Za-z0-9]){re.escape(name)}([^A-Za-z0-9]|$)", joined):
            return name
    return None
...
def infer_budget(*parts: str) -> int | None:
    joined = " ".join([p for p in parts if p])
    m = re.search(r"(?<![A-Za-z0-9])K(\d+)(?![A-Za-z0-9])", joined, flags=re.IGNORECASE)
    if m:
        return int(m.group(1))
    m = re.search(r"(?<![A-Za-z0-9])(\d+)T(?![A-Za-z0-9])", joined, flags=re.IGNORECASE)
    if m:
...
def infer_method(*parts: str) -> str | None:
    ...
    checks = [
        ...
        ("craig", "CRAIG"),
        ("less", "LESS"),
        ("source_plus_target", "FullSourceTarget"),
        ("s_and_t", "FullSourceTarget"),
        ("target_train", "TargetOnly"),
        ("source", "SourceOnly"),
    ]
...
def resolve_split_txt_dir(split_txt_dir: str | None, split: str) -> str | None:
    """Resolve old TAVO split paths after splits were moved under data/TAVO."""
    if not split_txt_dir:
...
def build_test_dataset(cfg: dict[str, Any]) -> BraTSSliceDataset:
    ...
    if not split_file_exists(split_txt_dir, split):
        ...
        raise FileNotFoundError(
            f"Required test split file is missing: {missing}. "
            "Refusing to evaluate all subjects accidentally."
        )
...
def make_case_row(
    ...
    row: dict[str, Any] = {
        ...
        "subset_seed": metadata.get("subset_seed"),
        "checkpoint": metadata.get("checkpoint_name"),
        "epoch": metadata.get("epoch"),
        "num_slices": num_slices,
        "volume_shape": "x".join(str(x) for x in vol_pred.shape),
    }
    ...
    for metric_name in METRICS:
        row[f"{metric_name}_macro"] = float(
            np.mean([region_metrics[name][metric_name] for name in REGIONS])
        )
...
def summarize(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for metric_name in METRICS:
        ...
        for region_name in list(REGIONS) + ["macro"]:
            ...
            summary["metrics"][metric_name][region_name] = {
    ...
    for region_name in REGIONS:
        summary["hd95_valid_counts"][region_name] = int(
            sum(bool(row[f"hd95_valid_{region_name}"]) for row in rows)
        )
    return summary
...
def attach_legacy_comparison(summary: dict[str, Any], legacy: dict[str, float] | None) -> None:
    ...
    for region_name, old_value in legacy.items():
        ...
        comparison[region_name] = {
            "legacy": old_value,
            "current": new_value,
            "abs_diff": abs(float(new_value) - float(old_value)),
        }
...
def main() -> None:
    ...
    metadata = {
        ...
        or infer_target_domain(config_text, checkpoint_text, save_dir, ckpt_parent),
        "method": args.method or infer_method(config_text, checkpoint_text, save_dir, ckpt_parent),
        "budget": args.budget if args.budget is not None else infer_budget(config_text, checkpoint_text, save_dir, ckpt_parent),
        "train_seed": args.train_seed,
        "search_seed": args.search_seed,
        "subset_seed": args.subset_seed or infer_seed(r"(?:repeat|random)(\d+)", config_text, checkpoint_text, save_dir),
        "epoch": checkpoint.get("epoch", checkpoint.get("global_iter")),
        "generated_at_utc": datetime.now(timezone.utc).isoformat(),
        "device": str(device),
        "use_amp": use_amp,
        "img_size": cfg["data"]["img_size"],
        "test_split": cfg["data"]["test"],
        "test_split_resolved": {
            **cfg["data"]["test"],
            "split_txt": cfg["data"]["test"].get("resolved_split_txt", cfg["data"]["test"].get("split_txt")),
        },
        "num_subjects": len(subject_to_slices),
        "hd95_units": "voxel",
        "empty_mask_convention": {
            "dice": "both empty=1; one empty=0",
            "hd95": "both empty=0; one empty=image diagonal penalty",
            "precision": "both empty=1; pred empty with nonempty GT=0",
            "recall": "both empty=1; GT empty with nonempty prediction=1",
        },
    }
    ...
    for subject_id, slice_list in tqdm(subject_to_slices.items(), desc="Evaluating subjects", ncols=100):
        vol_pred, vol_gt = reconstruct_subject(
    ...
    for region_name in list(REGIONS) + ["macro"]:
        print(
            f"{region_name:5s} "
            f"Dice={dice[region_name]['mean']:.4f} "
            f"HD95={hd95[region_name]['mean']:.4f}"
        )
    if "legacy_dice_comparison" in summary:
        print("\nLegacy Dice comparison:")
        for region_name, comp in summary["legacy_dice_comparison"].items():
            print(
                f"{region_name:5s} old={comp['legacy']:.4f} "
                f"new={comp['current']:.4f} diff={comp['abs_diff']:.6f}"
            )
