...
def orient_select_subset(src_subj_vecs, tgt_subj_vecs, src_subj_ids, budget, eta=1.0):
    ...
    result = obj.maximize(budget=min(budget, Ns), optimizer='LazyGreedy', stopIfNegativeGain=True, show_progress=True)
    selected_idx = []
    for elem in result:
        if isinstance(elem, tuple):
            selected_idx.append(elem[0])
        else:
            selected_idx.append(elem)
    selected_idx = sorted(selected_idx)
    return [src_subj_ids[i] for i in selected_idx]

def orient_select_source_subjects(model, device, base_dir, src_split_dir, tgt_split_dir, subset_size, img_size, batch_size, num_workers, eta=1.0, use_val_for_target=True, save_dir='results/orient_gradient'):
    os.makedirs(save_dir, exist_ok=True)
    theta_params = get_theta_params(model)
    src_dataset = BraTSSliceDataset(root_dir=os.path.join(base_dir, '002_BraTS21'), split='train', img_size=img_size, split_txt_dir=src_split_dir)
    src_dataset.return_meta = True
    tgt_train_dataset = BraTSSliceDataset(root_dir=os.path.join(base_dir, '002_BraTS21'), split='train', img_size=img_size, split_txt_dir=tgt_split_dir)
    tgt_train_dataset.return_meta = True
    if use_val_for_target:
        tgt_val_dataset = BraTSSliceDataset(root_dir=os.path.join(base_dir, '002_BraTS21'), split='val', img_size=img_size, split_txt_dir=tgt_split_dir)
    ...
    src_slice_g, src_slice_n = compute_gradient_features(model, src_loader, device, theta_params, save_prefix=os.path.join(save_dir, 'src'))
    tgt_slice_g, tgt_slice_n = compute_gradient_features(model, tgt_loader, device, theta_params, save_prefix=os.path.join(save_dir, 'tgt'))
    src_subj_ids, src_subj_vecs = aggregate_to_subject_level(src_slice_g, src_slice_n, save_prefix=os.path.join(save_dir, 'src'))
    tgt_subj_ids, tgt_subj_vecs = aggregate_to_subject_level(tgt_slice_g, tgt_slice_n, save_prefix=os.path.join(save_dir, 'tgt'))
    selected_ids = orient_select_subset(src_subj_vecs, tgt_subj_vecs, src_subj_ids, budget=subset_size, eta=eta)
    return selected_ids
