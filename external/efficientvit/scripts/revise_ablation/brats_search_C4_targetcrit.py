...
def get_template_yaml(repeat_id: int):
    return PROJECT_ROOT / 'configs_TAVO' / 'configs_C4_cma' / f'repeat{repeat_id:02d}' / 'template.yaml'
SCORE_ROOT = PROJECT_ROOT / 'data/TAVO/splits_C4_mix_scores_multi'
SPLIT_ROOT = PROJECT_ROOT / 'data/fusion_ablation/brats21/C4_stageA2'
OUT_ROOT = PROJECT_ROOT / 'outputs_fusion_ablation/brats21/C4_stageA2'
T = 50
...
def maybe_set_warmup(cfg, ckpt_path):
    ...
    return cfg
WARMUP_CKPT_MAP = {f'repeat{i:02d}': f'external/efficientvit/outputs_TAVO/outputs_C4_new/source_plus_target_repeat{i:02d}/best.pt' for i in range(1, 6)}
METHODS = ['rds', 'less', 'orient', 'gradmatch']
...
def run_candidate(repeat_id: int, score_dicts: Dict[str, Dict[str, float]], budget_T: int, run_tag: str, weights_vec: np.ndarray, iters: int, seeds: str, *, template_cfg: dict, full_source_dataset, target_dataset, val_dataset, subject_to_slice_indices) -> Dict[str, Any]:
    ...
    return {'repeat': repeat_id, 'weights': weights, 'iters': int(iters), 'fitness': float(fitness), 'dice_per_seed': dice_per_seed, 'out_dir': str(out_dir)}
...
def stageA2_cma(repeat_id: int, budget_T: int, run_tag: str, seeds_eval: str='0', seeds_refine: str='0,1,2', seed: int=0):
    print(' Prebuilding FULL datasets once...')
    template_yaml = get_template_yaml(repeat_id)
    template_cfg = remap_template_split_paths(yaml.safe_load(template_yaml.read_text()))
    ...
    print('\n==============================')
    print(' CMA-ES Stage A2 targetcrit')
    ...
    for c in corners:
        r = run_candidate(repeat_id, score_dicts, budget_T, run_tag, c, it_eval, seeds_eval, template_cfg=template_cfg, full_source_dataset=full_source_dataset, target_dataset=target_dataset, val_dataset=val_dataset, subject_to_slice_indices=subject_to_slice_indices)
    ...
    for g in range(n_gen):
        ...
        for k in range(lam):
            z = arx[k]
            r = run_candidate(repeat_id, score_dicts, budget_T, run_tag, z, it_eval, seeds_eval, template_cfg=template_cfg, full_source_dataset=full_source_dataset, target_dataset=target_dataset, val_dataset=val_dataset, subject_to_slice_indices=subject_to_slice_indices)
    ...
    for r in all_sorted[:A2_CFG['refine_topk']]:
        ...
        rr = run_candidate(repeat_id, score_dicts, budget_T, run_tag, vec, A2_CFG['iters_refine'], seeds_refine, template_cfg=template_cfg, full_source_dataset=full_source_dataset, target_dataset=target_dataset, val_dataset=val_dataset, subject_to_slice_indices=subject_to_slice_indices)
...
if __name__ == '__main__':
    ...
    out = stageA2_cma(args.repeat, args.budget_T, args.run_tag, args.seeds_eval, args.seeds_refine, args.seed)
