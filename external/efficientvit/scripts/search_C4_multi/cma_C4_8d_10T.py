...
def get_template_yaml(repeat_id: int):
    return PROJECT_ROOT / 'configs_C4_cma' / f'repeat{repeat_id:02d}' / 'template.yaml'
...
T = 50
WARMUP_CKPT_MAP = {f'repeat{i:02d}': f'external/efficientvit/outputs_C4_new/source_plus_target_repeat{i:02d}/epoch_007.pt' for i in range(1, 6)}
METHODS = ['rds', 'less', 'orient', 'craig', 'gradmatch', 'kmeans', 'kcenter', 'diversity']
A2_CFG = {'popsize': 20, 'mu': 10, 'n_gen': 12, 'sigma0': 0.3, 'sigma_min': 0.05, 'iters_eval': 1000, 'iters_refine': 2000, 'refine_topk': 8, 'max_subjects': 15, 'fitness_agg': 'median'}
