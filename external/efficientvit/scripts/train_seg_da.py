#!/usr/bin/env python3
"""Train BraTS EfficientViT domain-adaptation baselines.

This release entrypoint mirrors the config surface used for the paper runs:
DANN, DAN/MMD, ADVENT/AdvEnt, and SE-ASA. Source and labeled target-train
examples both receive supervised segmentation loss; the DA term is computed
between source and target-train batches.
"""
...
def entropy_map(logits: torch.Tensor) -> torch.Tensor:
    ...
    ent = -torch.sum(probs * torch.log(probs + 1e-6), dim=1, keepdim=True)
...
def make_dataset(data_cfg: dict[str, Any], key: str, default_skip: bool) -> BraTSSliceDataset:
    ...
    ds = BraTSSliceDataset(
        ...
        split_txt_dir=item.get("split_txt"),
        skip_empty=bool(data_cfg.get(f"skip_empty_{key}", default_skip)),
    )
...
def train_one_epoch(
    *,
    ...
    feature_disc: DomainDiscriminator | None,
    output_disc: PixelDiscriminator | None,
    source_loader: DataLoader,
    target_loader: DataLoader,
    optimizer: optim.Optimizer,
    output_optimizer: optim.Optimizer | None,
    criterion: nn.Module,
    device: torch.device,
        ...
        with torch.cuda.amp.autocast(enabled=(device.type == "cuda")):
            ...
            if method == "dann":
                ...
                da_loss = mmd_loss(
                    src_feats,
                    tgt_feats,
                    [float(x) for x in da_cfg.get("kernel_multipliers", [0.25, 0.5, 1.0, 2.0, 4.0])],
                    None if da_cfg.get("fixed_sigma", "auto") == "auto" else float(da_cfg["fixed_sigma"]),
                )
                ...
                if method in {"se_asa", "seasa"}:
                    ...
                    adv_loss = (
                        adv_loss
                        + float(da_cfg.get("seasa_lambda_class", 0.1)) * entropy_regularizer
                        + float(da_cfg.get("seasa_lambda_selective", 0.01)) * consistency
                    )
                da_loss = adv_loss
    ...
    return {
        "loss": totals["loss"] / max(totals["samples"], 1),
        "source_seg": totals["source_seg"] / max(source_seen, 1),
        "target_seg": totals["target_seg"] / max(target_seen, 1),
        "da_loss": totals["da_loss"] / max(totals["samples"], 1),
        "da_weight": totals["da_weight"] / max(steps, 1),
    }
...
def main(config_path: str) -> None:
    ...
    if da_method == "dann":
        feature_disc = DomainDiscriminator(
            feature_dim,
            int(cfg["da"].get("domain_hidden_dim", 256)),
            float(cfg["da"].get("domain_dropout", 0.5)),
        ).to(device)
    ...
    if output_disc is not None:
        output_optimizer = optim.Adam(
            output_disc.parameters(),
            lr=float(cfg["da"].get("lr_d", cfg["optimizer"]["lr"])),
            betas=(float(cfg["da"].get("beta1_d", 0.9)), float(cfg["da"].get("beta2_d", 0.99))),
        )
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
        optimizer,
        T_max=int(cfg["scheduler"]["T_max"]),
        eta_min=float(cfg["scheduler"].get("eta_min", 0.0)),
    )
    ...
    for epoch in range(epochs):
        ...
        scheduler.step()
        print(
            f"Train loss {train_stats['loss']:.4f} | Source seg {train_stats['source_seg']:.4f} | "
            f"Target seg {train_stats['target_seg']:.4f} | DA {train_stats['da_loss']:.4f} | "
            f"DA weight {train_stats['da_weight']:.4f} | Val loss {val_stats['loss']:.4f}"
        )
        checkpoint = {
            "epoch": epoch + 1,
            "model_state": model.state_dict(),
            "feature_domain_state": feature_disc.state_dict() if feature_disc is not None else None,
            "output_domain_state": output_disc.state_dict() if output_disc is not None else None,
