...
def make_loader(dataset, cfg, seed, shuffle):
    g = torch.Generator()
    g.manual_seed(seed)
    return DataLoader(dataset, batch_size=cfg['data']['batch_size'], shuffle=shuffle, generator=g if shuffle else None, num_workers=0, pin_memory=True, persistent_workers=False)

def train_short(model, loader, optimizer, criterion, device, scaler, start_iter, max_iters):
    model.train()
    losses = []
...
def run_short_training_with_dataset(cfg: dict, seeds: List[int], train_dataset, val_dataset) -> Dict[str, Any]:
    ...
    for seed in seeds:
        ...
        mcfg = cfg['model']
        model = EfficientViT_Seg(backbone=mcfg['name'], in_channels=mcfg['in_channels'], num_classes=mcfg['num_classes'], pretrained=ckpt is None).to(device)
        if ckpt is not None:
