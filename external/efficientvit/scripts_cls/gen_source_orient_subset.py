...
def maybe_load_source_cache(src_cache_root):
    ...
    required = [src_grad_path, src_name_path, src_vec_path, src_id_path]
    if not all((os.path.exists(p) for p in required)):
        return None
    out = {'src_grads': np.load(src_grad_path), 'src_names': load_txt(src_name_path), 'src_vecs': np.load(src_vec_path), 'src_ids': load_txt(src_id_path), 'src_K': np.load(src_K_path) if os.path.exists(src_K_path) else None}
    return out
...
def maybe_load_target_cache(tgt_cache_root):
    ...
    required = [tgt_grad_path, tgt_name_path, tgt_vec_path, tgt_id_path]
    if not all((os.path.exists(p) for p in required)):
        return None
    out = {'tgt_grads': np.load(tgt_grad_path), 'tgt_names': load_txt(tgt_name_path), 'tgt_vecs': np.load(tgt_vec_path), 'tgt_ids': load_txt(tgt_id_path), 'src_tgt_Q': np.load(tgt_Q_path) if os.path.exists(tgt_Q_path) else None}
    return out
...
def compute_gradient_features(model, loader, device, theta_params, save_prefix=None):
    ...
    for img, lbl, path in loader:
        ...
        diff = prob - onehot
        g = torch.einsum('bc,bd->bcd', diff, feat)
...
def main():
    ...
    ap.add_argument('--source_list', required=True, help='e.g. data_cls/splits/office31/amazon/source_train.txt')
    ap.add_argument('--target_list', required=True, help='e.g. data_cls/selections/office31/amazon/random1_2shot.txt')
    ap.add_argument('--warmup_ckpt', required=True, help='source-only warmup checkpoint, e.g. experiments_cls/office31/amazon/source_only_full/best.pt')
    ...
    np.save(os.path.join(tgt_cache_root, 'orient_score_dict.npy'), orient_score_dict, allow_pickle=True)
