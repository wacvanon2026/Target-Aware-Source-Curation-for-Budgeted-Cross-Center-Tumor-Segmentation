...
def build_final_config(
    ...
    return {
        "experiment": {
            "name": f"officehome_{target}_{method}_{budget_tag}_split00_train{train_seed:02d}",
            "save_dir": save_dir.as_posix(),
        },
        "data": {
            "num_classes": 65,
            "source_train": source_train.as_posix(),
            "source_val": target_val.as_posix(),
            "target_selected": target_selected.as_posix(),
            "target_test": target_test.as_posix(),
...
def make_loaders(source_txt, target_train_txt, target_val_txt, batch_size, num_workers):
    ...
    target_ds = OfficeDataset(
        target_train_txt,
        transform=train_transform,
        class_to_idx=source_ds.class_to_idx,
        return_path=False,
    )
    val_ds = OfficeDataset(
        target_val_txt,
...
def short_train_val(source_txt, target_train_txt, target_val_txt, args, seed):
    ...
    train_loader, val_loader = make_loaders(
        ...
        target_train_txt=target_train_txt,
        target_val_txt=target_val_txt,
...
def aggregate_short_train_val(source_txt, target_train_txt, target_val_txt, args, seeds):
    ...
    last_vals = []
    for seed in seeds:
        best_val, last_val = short_train_val(
            source_txt=source_txt,
            target_train_txt=target_train_txt,
            target_val_txt=target_val_txt,
            args=args,
    ...
    return {
        "fitness": float(np.median(best_vals)),
        "mean_best_val_acc": float(np.mean(best_vals)),
        "last_val_acc": float(np.median(last_vals)),
        "per_seed": per_seed,
    }
...
def run_candidate(
    ...
    selected, weights, rank_csv = build_tavo_subset(
    ...
    agg = aggregate_short_train_val(
        source_txt=subset_txt,
        target_train_txt=paths["target_train"],
        target_val_txt=paths["target_val"],
        args=args,
        seeds=seeds,
    )
    ...
    return {
        ...
        "iter_tag": iter_tag,
        "weights": weights,
        "z": weights_vec.tolist(),
        "fitness": float(agg["fitness"]),
        "mean_best_val_acc": float(agg["mean_best_val_acc"]),
        "last_val_acc": float(agg["last_val_acc"]),
        "per_seed": agg["per_seed"],
        "subset_path": subset_txt.as_posix(),
        "ranking_path": rank_csv.as_posix(),
...
def write_final_config(target, method_name, source_subset, paths, args, train_seed):
    ...
    cfg_dir = (
        paths["final_config_root"] / target / method_key / f"B{args.budget_per_class}" /
        f"split{args.split_seed:02d}"
    )
    out_dir = (
        paths["final_output_root"] / target / method_key / f"B{args.budget_per_class}" /
        f"split{args.split_seed:02d}" / f"train_seed{train_seed:02d}"
    )
    cfg_path = cfg_dir / f"train_seed{train_seed:02d}.yaml"
    cfg = build_final_config(
        target=target,
        method=method_key,
        source_train=source_subset,
        target_selected=paths["target_train"],
        target_val=paths["target_val"],
        target_test=paths["target_test"],
        save_dir=out_dir,
...
def run_final_training(configs):
    ...
    for method_name, cfg_path, out_dir in configs:
        ...
        if last_best.exists():
            ...
            continue
        subprocess.run(
            ["python", "-u", "scripts_cls/train_cls.py", "--config", cfg_path.as_posix()],
            cwd=PROJECT_ROOT,
            env=env,
            check=True,
        )
...
def build_paths(args):
    ...
    rank_root = (
        PROJECT_ROOT / args.rank_root / args.target / f"seed{args.split_seed:02d}" /
        f"B{args.budget_per_class}"
    )
    search_root = (
        PROJECT_ROOT / args.search_output_root / args.target /
        f"B{args.budget_per_class}" / f"search_seed{args.search_seed:02d}"
    )
    return {
        "source_train": split_dir / "source_train.txt",
        "target_train": split_dir / "target_train_3shot.txt",
        "target_val": split_dir / "target_val_2shot.txt",
        "target_test": split_dir / "target_test.txt",
        "rank_root": rank_root,
        "search_root": search_root,
        "search_subset_root": search_root / "candidate_subsets",
        "final_subset_root": PROJECT_ROOT / args.final_subset_root,
        "final_config_root": PROJECT_ROOT / args.final_config_root,
        "final_output_root": PROJECT_ROOT / args.final_output_root,
    }
...
def run_search(args):
    ...
    if args.dry_run:
        ...
        payload = {
            ...
            "final_config": cfg_path.as_posix(),
            "final_output": out_dir.as_posix(),
        }
    ...
    for gen in range(args.n_gen):
        ...
        hsig = 1.0 if (
            ps_norm / math.sqrt(1 - (1 - cs) ** (2 * (gen + 1)))
            < (1.4 + 2 / (dim + 1)) * chi_n
        ) else 0.0
        ...
        cov = (
            (1 - c1 - cmu) * cov
            + c1 * (pc[:, None] @ pc[None, :] + (1 - hsig) * cc * (2 - cc) * cov)
            + cmu * rank_mu
        )
    ...
    for rank, record in enumerate(refine_inputs):
        ...
        print(
            f"[refine] origin={record['id']} refined_id={eval_id} "
            f"fitness={refined['fitness']:.4f}"
        )
        eval_id += 1
    ...
    out_payload = {
        ...
        "tavo_8d_record": refine_records[0],
        "tavo_8d_best_record": best_origin_refine,
    }
    ...
    final_configs = []
    variants = [
        ("TAVO_8D", refine_records[0]),
        ("TAVO_8D_best", best_origin_refine),
    ]
    for method_name, record in variants:
        ...
        for train_seed in final_train_seeds:
            ...
            final_manifest.append({
                "target": args.target,
                "method": method_name,
                "budget": f"B{args.budget_per_class}",
                "train_seed": train_seed,
                "config": cfg_path.as_posix(),
                "source_subset": subset.as_posix(),
                "search_json": json_path.as_posix(),
                "category": "tavo",
            })
