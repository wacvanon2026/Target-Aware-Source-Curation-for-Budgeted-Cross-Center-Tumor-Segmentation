...
EFFICIENTVIT_ROOT = SCRIPT_DIR.parent
...
def run_final_training(configs):
    env = os.environ.copy()
    env['PYTHONPATH'] = str(EFFICIENTVIT_ROOT) + os.pathsep + env.get('PYTHONPATH', '')
    ...
    for method_name, cfg_path, out_dir in configs:
        ...
        if last_best.exists():
            ...
            continue
        subprocess.run(['python', '-u', str(SCRIPT_DIR / 'train_cls.py'), '--config', cfg_path.as_posix()], cwd=PROJECT_ROOT, env=env, check=True)
