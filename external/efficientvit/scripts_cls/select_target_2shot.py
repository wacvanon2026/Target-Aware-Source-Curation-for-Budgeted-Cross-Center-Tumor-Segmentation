...
# =====================================================
# Metric Projector (trained on SOURCE only)
# =====================================================
...
class LabelBalancedBatchSampler(Sampler):
    ...
    def __iter__(self):
        for _ in range(self.num_batches):
            chosen_classes = self.rng.sample(
                self.classes,
                k=min(self.classes_per_batch, len(self.classes))
            )
            batch = []
...
def train_metric_projector_on_source(
    source_embeddings,
    source_labels,
    epochs=50,
    lr=1e-3,
...
# =====================================================
# Selection helpers (per-class balanced: select k each class)
# =====================================================
...
def facility_location_greedy(x, k):
    """
    CRAIG-like facility location greedy selection.
    ...
    # cosine similarity matrix (more stable than dot product)
    sim = cosine_similarity(x)

    selected = []

    # current coverage
    current = np.full(n, -1e9, dtype=np.float32)
...
def main():
    ...
    # metric projector params
    parser.add_argument("--metric_epochs", type=int, default=30)
    parser.add_argument("--metric_lr", type=float, default=1e-3)
    parser.add_argument("--metric_wd", type=float, default=1e-4)
    ...
    if args.use_saved_embeddings and os.path.exists(src_embed_path) and os.path.exists(tgt_embed_path):
        ...
        src_emb, src_y, src_paths, src_cls, label_to_idx = extract_embeddings(
            feature_model,
            source_items,
        ...
        tgt_emb, tgt_y, tgt_paths, tgt_cls, _ = extract_embeddings(
            feature_model,
            target_items,
            batch_size=args.batch_size,
            num_workers=args.num_workers,
    ...
    # -------------------------
    # metric projector on SOURCE only
    # -------------------------
    # print("Training metric projector on SOURCE embeddings...")
    # projector = train_metric_projector_on_source(
    #     source_embeddings=src_emb,
    #     source_labels=src_y,
    #     epochs=args.metric_epochs,
    #     lr=args.metric_lr,
    #     wd=args.metric_wd,
    #     hidden_dim=args.metric_hidden_dim,
    #     out_dim=args.metric_out_dim,
    #     seed=args.seed
    # )

    # print("Projecting TARGET embeddings into metric space...")
    # tgt_metric_emb = project_embeddings(projector, tgt_emb)
    # np.save(os.path.join(args.output_dir, "target_metric_embeddings.npy"), tgt_metric_emb)

    # -------------------------
    # balanced target selectors
    # NOTE:
    # uses hidden class labels ONLY to enforce 2-shot/class protocol
    # -------------------------
    k = args.budget_per_class

    # print("Selecting Random (3 seeds)...")
    # random1 = select_random_per_class(target_items, k, args.seed)
    # random2 = select_random_per_class(target_items, k, args.seed + 1)
    # random3 = select_random_per_class(target_items, k, args.seed + 2)

    # save_selected_txt(random1, os.path.join(args.output_dir, f"random1_{k}shot.txt"))
    # save_selected_txt(random2, os.path.join(args.output_dir, f"random2_{k}shot.txt"))
    # save_selected_txt(random3, os.path.join(args.output_dir, f"random3_{k}shot.txt"))

    # print("Selecting KMeans...")
    # kmeans_sel = select_kmeans_per_class(target_items, tgt_emb, k, args.seed)
    # save_selected_txt(kmeans_sel, os.path.join(args.output_dir, f"kmeans_{k}shot.txt"))

    # print("Selecting Coreset...")
    # coreset_sel = select_coreset_per_class(target_items, tgt_emb, k)
    # save_selected_txt(coreset_sel, os.path.join(args.output_dir, f"coreset_{k}shot.txt"))

    # print("Selecting Metric-Coreset...")
    # metric_coreset_sel = select_metric_coreset_per_class(target_items, tgt_metric_emb, k)
    # save_selected_txt(metric_coreset_sel, os.path.join(args.output_dir, f"metric_coreset_{k}shot.txt"))
