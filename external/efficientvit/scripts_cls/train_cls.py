...
def main(config_path):
    ...
    shutil.copy(
        os.path.join(save_dir,f"epoch_{last_epoch:03d}.pt"),
    ...
    if use_val:
        ...
        best_recent = sorted(
            recent,
            key=lambda x:(-x["val_acc"],x["val_loss"])
        )[0]
    ...
    shutil.copy(
        os.path.join(save_dir,f"epoch_{last_best_epoch:03d}.pt"),
        os.path.join(save_dir,"last_best.pt")
    )
    ...
    # print("✔ Removed epoch checkpoints")
