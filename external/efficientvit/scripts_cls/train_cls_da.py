...
def entropy_weights(logits: torch.Tensor) -> torch.Tensor:
    ...
    ent = -torch.sum(probs * torch.log(probs + 1e-06), dim=1)
    weights = 1.0 + torch.exp(-ent)
    return weights / weights.mean().detach().clamp_min(1e-06)
...
def make_loader(dataset, cfg: dict[str, Any], shuffle: bool, drop_last: bool, generator: torch.Generator) -> DataLoader:
    return DataLoader(dataset, batch_size=cfg['data']['batch_size'], shuffle=shuffle, num_workers=cfg['data']['num_workers'], pin_memory=True, drop_last=drop_last, worker_init_fn=seed_worker, generator=generator)
...
def evaluate(model: ResNet50Classifier, loader: DataLoader, criterion: nn.Module, device: torch.device, desc: str) -> dict[str, float]:
    ...
    return {'loss': total_loss / max(total_samples, 1), 'acc': total_correct / max(total_samples, 1), 'macro_f1': macro_f1, 'balanced_acc': balanced_acc}

def train_one_epoch(*, model: ResNet50Classifier, domain_disc: DomainDiscriminator | None, random_layer: RandomLayer | None, source_loader: DataLoader, target_loader: DataLoader, optimizer: optim.Optimizer, criterion: nn.Module, device: torch.device, scaler: torch.cuda.amp.GradScaler, cfg: dict[str, Any], epoch: int) -> dict[str, float]:
    model.train()
    ...
    total = {'loss': 0.0, 'source_ce': 0.0, 'target_ce': 0.0, 'da_loss': 0.0, 'da_weight': 0.0, 'source_correct': 0, 'target_correct': 0, 'samples': 0}
    ...
    for step in tqdm(range(steps), desc='Training DA', ncols=100):
        ...
        with torch.cuda.amp.autocast(enabled=device.type == 'cuda'):
            ...
            if method == 'dann':
                ...
                da_loss = mmd_loss(src_feats, tgt_feats, multipliers=[float(x) for x in da_cfg.get('kernel_multipliers', [0.25, 0.5, 1.0, 2.0, 4.0])], fixed_sigma=None if da_cfg.get('fixed_sigma', 'auto') == 'auto' else float(da_cfg['fixed_sigma']))
                ...
                labels = torch.cat([torch.zeros(src_joint.size(0), device=device), torch.ones(tgt_joint.size(0), device=device)])
                weights = None
    ...
    return {'loss': total['loss'] / max(total['samples'], 1), 'source_ce': total['source_ce'] / max(source_seen, 1), 'target_ce': total['target_ce'] / max(target_seen, 1), 'da_loss': total['da_loss'] / max(total['samples'], 1), 'da_weight': total['da_weight'] / max(steps, 1), 'source_acc': total['source_correct'] / max(source_seen, 1), 'target_acc': total['target_correct'] / max(target_seen, 1)}
...
def main(config_path: str) -> None:
    ...
    for epoch in range(epochs):
        ...
        train_stats = train_one_epoch(model=model, domain_disc=domain_disc, random_layer=random_layer, source_loader=source_loader, target_loader=target_loader, optimizer=optimizer, criterion=criterion, device=device, scaler=scaler, cfg=cfg, epoch=epoch)
        val_stats = evaluate(model, val_loader, criterion, device, 'Validating target_val')
        scheduler.step()
        print(f"Train loss {train_stats['loss']:.4f} | Source acc {train_stats['source_acc']:.4f} | Target train acc {train_stats['target_acc']:.4f} | DA raw {train_stats['da_loss']:.4f} | DA weight {train_stats['da_weight']:.4f} | Val acc {val_stats['acc']:.4f} | Val macroF1 {val_stats['macro_f1']:.4f}")
        checkpoint = {'epoch': epoch + 1, 'model_state': model.state_dict(), 'domain_state': domain_disc.state_dict() if domain_disc is not None else None, 'optimizer_state': optimizer.state_dict(), 'scheduler_state': scheduler.state_dict(), 'train_stats': train_stats, 'val_stats': val_stats, 'val_acc': val_stats['acc'], 'val_loss': val_stats['loss'], 'config': cfg}
    ...
    for ckpt_name in ['best.pt', 'last.pt', 'last_best.pt']:
        ...
        print(f"{ckpt_name} Target Acc: {stats['acc']:.4f} | Macro-F1: {stats['macro_f1']:.4f} | Balanced Acc: {stats['balanced_acc']:.4f}")
