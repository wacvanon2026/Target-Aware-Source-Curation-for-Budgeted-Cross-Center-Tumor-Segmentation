...
METHODS = ['KMeans-B', 'KCenter-B', 'FacilityLocation-B', 'CRAIG-B']
...
def run_search(args):
    ...
    best_so_far = None
    eval_id = 0
    print('===== OfficeHome TAVO_SourceCriteria fusion ablation search =====')
    ...
    if args.dry_run:
        ...
        final_subset = finalize_subset(record, args.target, 'TAVO_SourceCriteria', paths, args)
        cfg_path, out_dir = write_final_config(args.target, 'TAVO_SourceCriteria', final_subset, paths, args, args.train_seed)
    ...
    final_manifest = []
    final_configs = []
    variants = [('TAVO_SourceCriteria', refine_records[0])]
