...
METHODS = ['TargetMMD-B', 'TargetGradMatch-B', 'GLISTER-B', 'ORIENT-B']
...
def run_search(args):
    ...
    best_so_far = None
    eval_id = 0
    print('===== OfficeHome TAVO_TargetCriteria fusion ablation search =====')
    ...
    if args.dry_run:
        ...
        final_subset = finalize_subset(record, args.target, 'TAVO_TargetCriteria', paths, args)
        cfg_path, out_dir = write_final_config(args.target, 'TAVO_TargetCriteria', final_subset, paths, args, args.train_seed)
    ...
    final_manifest = []
    final_configs = []
    variants = [('TAVO_TargetCriteria', refine_records[0])]
