...
from core import EXPERIMENTS, TARGET_BASES, Experiment, dataset_id, dataset_name, dataset_root, expand_experiments, nnunet_raw_root, normalize_target, split_root
...
def validate_cases(images_root: Path, labels_root: Path, train_cases: list[str], val_cases: list[str], test_cases: list[str], num_phases: int) -> None:
...
def build_one(args: argparse.Namespace, target: str, exp: Experiment) -> dict[str, object]:
    ...
    metadata = {'target': target, 'experiment': exp.key, 'dataset_id': ds_id, 'dataset_name': ds_name, 'dataset_dir': str(dataset_dir), 'description': exp.description, 'train_cases': len(train_cases), 'val_cases': len(val_cases), 'test_cases': len(test_cases), 'train_source': exp.train_source, 'split_root': str(args.split_root)}
    if args.dry_run:
    ...
    for case_id in test_cases:
        for image in source_image_paths(images_root, case_id, args.num_phases):
            ...
            op_counts[op] += 1
    dataset_json = {'channel_names': channel_names(args.num_phases), 'labels': {'background': 0, 'tumor': 1}, 'numTraining': len(train_cases) + len(val_cases), 'file_ending': '.nii.gz', 'dataset_name': ds_name.removesuffix('_2d_3ch')}
    ...
    dataset_dir.joinpath('tavo_mamamia_manifest.json').write_text(json.dumps(metadata | {'link_mode': args.link_mode, 'file_operations': op_counts}, indent=2) + '\n')
...
def main() -> None:
    ...
    summaries = [build_one(args, target, EXPERIMENTS[exp_key]) for target in requested_targets for exp_key in requested_exps]
    print('Summary:')
