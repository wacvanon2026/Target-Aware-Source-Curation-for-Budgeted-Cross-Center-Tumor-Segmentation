...
def split_counts(n: int) -> dict[str, int]:
    ...
    remainders = {name: n * weight / denom - floors[name] for name, weight in RATIO.items()}
...
def collect_domains(images_root: Path) -> dict[str, list[str]]:
    ...
    unknown = sorted((case_id for case_id in all_case_dirs if not any((case_id.startswith(prefix) for prefix in DOMAINS.values()))))
    if unknown:
...
def build_target_split(target: str, domains: dict[str, list[str]], seed: int) -> dict[str, object]:
    ...
    source_pool = sorted((case_id for domain, cases in domains.items() if domain != target for case_id in cases))
    ...
    return {'target': target, 'seed': seed, 'ratio': RATIO, 'target_total': len(target_cases), 'source_total': len(source_pool), 'counts': {'target_train': len(target_train), 'target_val': len(target_val), 'target_test': len(target_test)}, 'target_train': target_train, 'target_val': target_val, 'target_test': target_test, 'source_pool': source_pool, 'random': random_by_budget}
...
def main() -> None:
    ...
    summary = {'seed': args.seed, 'ratio': RATIO, 'domain_counts': {domain: len(cases) for domain, cases in domains.items()}, 'targets': {}}
    for target in targets:
        ...
        serializable = {key: value for key, value in split.items() if key not in {'target_train', 'target_val', 'target_test', 'source_pool', 'random'}}
        ...
        print(f"{target}: target={split['target_total']} train={split['counts']['target_train']} val={split['counts']['target_val']} test={split['counts']['target_test']} source={split['source_total']}")
