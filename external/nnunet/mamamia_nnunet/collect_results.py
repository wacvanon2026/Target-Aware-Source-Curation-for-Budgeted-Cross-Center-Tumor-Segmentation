#!/usr/bin/env python3
"""Collect completed MAMAMIA nnUNet summary.json files into a table."""
...
BASELINE_EXPERIMENTS = (
    ...
    ("source_only", "Source-only Full"),
    ("target_full_source", "Target + Full Source"),
)
DOMAIN_ALIGNMENT_METHODS = (
    ("dann", "DANN"),
    ("mmd", "MMD / DAN"),
    ("advent", "ADVENT"),
    ("seasa", "SE-ASA"),
...
RANDOM_EXPERIMENTS = {
    50: (("random50", "Random"),),
    150: (("random150", "Random"),),
    250: (("random250", "Random"),),
}
DOMAIN_ALIGNMENT_BUDGET_EXPERIMENTS = {
    budget: tuple(
        (f"random{budget}_{method}", label)
        for method, label in DOMAIN_ALIGNMENT_METHODS
    )
    for budget in (50, 150, 250)
}
DOMAIN_ALIGNMENT_EXPERIMENT_KEYS = {
    experiment
    for experiments in DOMAIN_ALIGNMENT_BUDGET_EXPERIMENTS.values()
...
OLD_RESULTS = {
    "target_only": {"NACT": 0.4740, "ISPY1": 0.5788, "DUKE": 0.5103, "ISPY2": 0.6126},
    "source_only": {"NACT": 0.6860, "ISPY1": 0.7087, "DUKE": 0.5776, "ISPY2": 0.6686},
    "target_full_source": {"NACT": 0.6810, "ISPY1": 0.7158, "DUKE": 0.5302, "ISPY2": 0.6943},
    "rds250": {"NACT": 0.6938, "ISPY1": 0.7107, "DUKE": 0.6338, "ISPY2": 0.6914},
    "gradmatch250": {"NACT": 0.6904, "ISPY1": 0.7170, "DUKE": 0.6393, "ISPY2": 0.6906},
    "less250": {"NACT": 0.7086, "ISPY1": 0.7202, "DUKE": 0.6310, "ISPY2": 0.7015},
    "orient250": {"NACT": 0.6167, "ISPY1": 0.7093, "DUKE": 0.6127, "ISPY2": 0.6925},
    "diversity250": {"NACT": 0.6417, "ISPY1": 0.7064, "DUKE": 0.5896, "ISPY2": 0.6827},
    "kmeans250": {"NACT": 0.6744, "ISPY1": 0.7206, "DUKE": 0.6120, "ISPY2": 0.6851},
    "craig250": {"NACT": 0.5497, "ISPY1": 0.6965, "DUKE": 0.5984, "ISPY2": 0.6971},
    "kcenter250": {"NACT": 0.6492, "ISPY1": 0.7175, "DUKE": 0.6250, "ISPY2": 0.6826},
...
def collect_rows(
    ...
    include_domain_alignment_results: bool = False,
    metric: str = "Dice",
) -> list[dict[str, str]]:
    rows = []
    metric = metric.upper()
    for experiment, label in experiments:
        if experiment in DOMAIN_ALIGNMENT_EXPERIMENT_KEYS and not include_domain_alignment_results:
            ...
            values = {
                target: first_foreground_metric(project_roots, target, experiment, metric)
        ...
        rows.append(
            {
                ...
                **{
                    target: fmt_cell(experiment, target, values[target], old_values.get(target), metric)
                    for target in TARGETS
                },
                "Avg": fmt_value_pair(avg, old_avg) if metric == "DICE" else fmt(avg),
                "completed": str(len(completed) if completed else len(old_completed)),
            }
        )
    return rows
...
def collect(
    ...
    sections = {
        "baselines": collect_rows(project_roots, BASELINE_EXPERIMENTS, metric=metric),
    }
    for budget in (50, 150, 250):
        rows = collect_rows(project_roots, RANDOM_EXPERIMENTS[budget], metric=metric)
        if budget in METHOD_BUDGETS:
            method_experiments = tuple(
                (f"{method}{budget}", METHOD_LABELS[method])
                for method in METHODS
            ) + ((f"tavo{budget}", "TAVO"),)
            rows.extend(collect_rows(project_roots, method_experiments, metric=metric))
        elif budget == 250:
            method_experiments = tuple(
                (f"{method}250", METHOD_LABELS[method])
                for method in METHODS
            ) + (("tavo250", "TAVO"),)
            rows.extend(collect_rows(project_roots, method_experiments, metric=metric))
        sections[f"k{budget}"] = rows
        sections[f"da_k{budget}"] = collect_rows(
            project_roots,
            DOMAIN_ALIGNMENT_BUDGET_EXPERIMENTS[budget],
            include_domain_alignment_results,
            metric,
        )
    return sections
...
def numeric_from_cell(cell: str) -> float | None:
    if not cell:
        return None
    match = re.search(r"new:\s*([0-9.]+)", cell)
    if match:
...
def preserve_existing_domain_alignment_cells(
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        action="append",
        default=[],
        help="Additional project root(s) to search after --project-root, useful for scratch DA runs.",
    )
    ...
    parser.add_argument(
        "--append-hd95",
        action="store_true",
        help="When writing a Dice Markdown report, append separate HD95 tables after the Dice tables.",
    )
    parser.add_argument(
        "--include-domain-alignment-results",
        action="store_true",
        help="Fill DA rows from existing summaries. Default keeps DA rows blank because they are run externally.",
    )
    ...
    if args.markdown:
        ...
        with contextlib.redirect_stdout(buffer):
            ...
            if extra_project_roots:
                print(
                    "Additional result roots: "
                    + ", ".join(f"`{root / 'outputs'}`" for root in extra_project_roots)
                    + "."
                )
            if args.metric == "HD95":
                print(
                    "Values are foreground mean HD95 from each `test_preds/summary_hd95.json`; "
                    "lower is better and blank cells are not complete yet."
                )
            else:
            ...
            if args.append_hd95 and args.metric == "Dice":
                hd95_sections = collect(
                    project_root,
                    args.include_domain_alignment_results,
                    "HD95",
                    extra_project_roots,
                )
                print()
