...
def output_dir(project_root_: Path, target: str, experiment: str) -> Path:
    return project_root_ / 'outputs' / f'tavo_mamamia_{target.lower()}_nnunet_{experiment}' / 'repeat_01'
...
def compute_one(pred_dir: Path, gt_dir: Path, output_path: Path, workers: int) -> dict:
    ...
    result = {'foreground_mean': {'HD95': float(np.mean(values))}, 'mean': {'1': {'HD95': float(np.mean(values))}}, 'metric_per_case': per_case, 'num_cases': len(values)}
