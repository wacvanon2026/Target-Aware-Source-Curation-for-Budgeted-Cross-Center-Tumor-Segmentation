...
TARGET_INPUTS = {'NACT': {'embedding_root': PROJECT_ROOT / 'dataset_mamamia' / 'embeddings', 'gradient_root': PROJECT_ROOT / 'mamamia_clean' / 'data' / 'gradients'}, 'ISPY1': {'embedding_root': PROJECT_ROOT / 'mamamia_ispy1' / 'data' / 'embeddings', 'gradient_root': PROJECT_ROOT / 'mamamia_ispy1' / 'data' / 'gradients'}, 'DUKE': {'embedding_root': PROJECT_ROOT / 'mamamia_duke' / 'data' / 'embeddings', 'gradient_root': PROJECT_ROOT / 'mamamia_duke' / 'data' / 'gradients'}, 'ISPY2': {'embedding_root': PROJECT_ROOT / 'mamamia_ispy2' / 'data' / 'embeddings', 'gradient_root': PROJECT_ROOT / 'mamamia_ispy2' / 'data' / 'gradients'}}
...
def write_selection(output_root: Path, target: str, method: str, ids: list[str], order: list[int], scores: dict[str, float], budget: int, metadata: dict[str, Any]) -> Path:
    selected = [ids[idx] for idx in order[:budget]]
    out_dir = output_root / target
    out_dir.mkdir(parents=True, exist_ok=True)
    out_path = out_dir / f'{method}_{budget}.json'
    payload = {'method': method, 'lineage': 'tumorseg2025', 'budget': budget, 'selected': selected, 'selected_cases': selected, 'scores': scores, 'metadata': metadata}
...
def orient_order(pool_ids: list[str], pool_vecs: np.ndarray, query_vecs: np.ndarray, budget: int, eta: float=1.0) -> tuple[list[int], dict[str, float], str]:
    ...
    if FacilityLocationMutualInformationFunction is not None:
        ...
        obj = FacilityLocationMutualInformationFunction(n=src.shape[0], num_queries=tgt.shape[0], data_sijs=data_sijs, query_sijs=query_sijs, magnificationEta=eta)
        result = obj.maximize(budget=min(budget, src.shape[0]), optimizer='LazyGreedy', stopIfNegativeGain=False, show_progress=True)
...
def craig_order(pool_ids: list[str], pool_vecs: np.ndarray, budget: int) -> tuple[list[int], dict[str, float], str]:
    ...
    if FacilityLocationFunction is not None:
        obj = FacilityLocationFunction(n=src.shape[0], mode='dense', sijs=sim.astype(np.float64), separate_rep=False)
        result = obj.maximize(budget=min(budget, src.shape[0]), optimizer='LazyGreedy', stopIfNegativeGain=False, show_progress=True)
        selected: list[int] = []
        gains: list[float] = []
        for elem in result:
...
def derive_target(args: argparse.Namespace, target: str) -> list[Path]:
    ...
    if gradients is None:
        ...
        if missing_gradient_methods and (not args.allow_missing_gradients):
            raise SystemExit(f"{target}: missing gradient arrays under {inputs['gradient_root']} for {', '.join(missing_gradient_methods)}. Generate gradients or rerun with --allow-missing-gradients to skip these artifacts.")
    ...
    if gradients is not None:
        ...
        if 'orient' in methods:
            ...
            output_paths.append(write_selection(args.output_root, target, 'orient', grad_pool_ids, order, scores, args.max_rank, {'rule': 'facility_location_mi_greedy', 'backend': backend, 'eta': args.orient_eta, 'source': 'gradients', 'gradient_root': str(inputs['gradient_root'])}))
        if 'less' in methods:
            order, scores = less_order(grad_pool_ids, pool_grads, query_grads, beta=args.less_beta)
            output_paths.append(write_selection(args.output_root, target, 'less', grad_pool_ids, order, scores, args.max_rank, {'rule': 'beta_weighted_gradient_cosine', 'beta': args.less_beta, 'source': 'gradients'}))
        if 'gradmatch' in methods:
            order, scores = gradmatch_order(grad_pool_ids, pool_grads, query_grads, args.max_rank)
            output_paths.append(write_selection(args.output_root, target, 'gradmatch', grad_pool_ids, order, scores, args.max_rank, {'rule': 'positive_omp_mean_target_gradient', 'source': 'gradients', 'normalize': True}))
        if 'craig' in methods:
            order, scores, backend = craig_order(grad_pool_ids, pool_grads, args.max_rank)
            output_paths.append(write_selection(args.output_root, target, 'craig', grad_pool_ids, order, scores, args.max_rank, {'rule': 'source_gradient_facility_location', 'backend': backend, 'source': 'gradients', 'normalize': True}))
    if gradients is None and args.allow_missing_gradients and ('orient' in methods):
        ...
        output_paths.append(write_selection(args.output_root, target, 'orient', pool_ids, order, scores, args.max_rank, {'rule': 'facility_location_mi_greedy', 'backend': backend, 'eta': args.orient_eta, 'source': 'embeddings_fallback'}))
    if gradients is None and args.allow_missing_gradients and ('gradmatch' in methods):
        ...
        output_paths.append(write_selection(args.output_root, target, 'gradmatch', pool_ids, order, scores, args.max_rank, {'rule': 'positive_omp_mean_target_embedding', 'source': 'embeddings_fallback', 'normalize': True}))
    if gradients is None and args.allow_missing_gradients and ('craig' in methods):
        ...
        output_paths.append(write_selection(args.output_root, target, 'craig', pool_ids, order, scores, args.max_rank, {'rule': 'source_embedding_facility_location', 'backend': backend, 'source': 'embeddings_fallback', 'normalize': True}))
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--allow-missing-gradients', action='store_true', help='Skip or fallback for gradient methods when a target lacks gradient arrays.')
