#!/usr/bin/env python3
"""Extract per-case projected nnUNet gradients for MAMAMIA selections."""
...
from meta.fitness.nnunet_proxy import DCandCELoss, create_nnunet_2d_model  # noqa: E402
...
def main() -> None:
    ...
    pool_ids, pool_gradients = extract_set(
        "pool", pool_cases, data_dirs, model, criterion, params, device, hash_indices, signs, args
    )
    query_ids, query_gradients = extract_set(
        "query", query_cases, data_dirs, model, criterion, params, device, hash_indices, signs, args
    )
    ...
    (args.output_dir / "info.json").write_text(
        json.dumps(
            {
                "checkpoint": str(args.checkpoint),
                "dataset_dirs": [str(path) for path in data_dirs],
                "grad_dim_raw": grad_dim,
