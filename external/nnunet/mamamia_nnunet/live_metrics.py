...
JOB_RE = re.compile('^(?P<name>.+)_(?P<job_id>\\d+)\\.out$')
...
def prediction_progress(name: str) -> str:
    ...
    pred_dir = project_root / 'outputs' / f'tavo_mamamia_{target_lc}_nnunet_{experiment}' / 'repeat_01' / 'test_preds'
...
def queue_entries() -> dict[str, QueueEntry]:
    try:
        output = subprocess.check_output(['squeue', '-u', subprocess.check_output(['whoami'], text=True).strip(), '-h', '-o', '%i|%j|%T|%M|%R'], text=True, stderr=subprocess.DEVNULL)
...
def parse_log(path: Path, queue: dict[str, QueueEntry]) -> Metrics:
    ...
    metrics = Metrics(log_path=path, job_id=job_id, name=name, state=entry.state if entry else '', elapsed=entry.elapsed if entry else '', predictions=prediction_progress(name))
...
def print_table(rows: list[Metrics]) -> None:
    headers = ('job', 'state', 'time', 'name', 'epoch', 'lr', 'train', 'val', 'pseudo', 'best', 'test', 'preds', 'stage')
    data = [(row.job_id, row.state, row.elapsed, row.name.replace('mamamia_', '', 1), row.epoch, row.learning_rate, row.train_loss, row.val_loss, row.pseudo_dice, row.best_ema_pseudo_dice, row.test_dice, row.predictions, row.last_stage) for row in rows]
