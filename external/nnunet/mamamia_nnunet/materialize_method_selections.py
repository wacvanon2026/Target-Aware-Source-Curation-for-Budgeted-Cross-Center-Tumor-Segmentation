...
TARGET_SELECTION_DIRS = {'NACT': SELECTION_ROOT / 'mamamia_clean' / 'outputs' / 'selections', 'ISPY1': SELECTION_ROOT / 'mamamia_ispy1' / 'outputs' / 'selections', 'DUKE': SELECTION_ROOT / 'mamamia_duke' / 'outputs' / 'selections', 'ISPY2': SELECTION_ROOT / 'mamamia_ispy2' / 'outputs' / 'selections'}
...
def real_tavo_selection_paths(target: str, budget: int) -> list[Path]:
    target_roots = {'NACT': SELECTION_ROOT / 'mamamia_clean', 'ISPY1': SELECTION_ROOT / 'mamamia_ispy1', 'DUKE': SELECTION_ROOT / 'mamamia_duke', 'ISPY2': SELECTION_ROOT / 'mamamia_ispy2'}
    global_meta_dir = SELECTION_ROOT / 'outputs' / 'meta'
    fresh_8d_candidates: list[Path] = []
    if global_meta_dir.exists():
        fresh_8d_candidates = list(global_meta_dir.glob(f'tavo_8d_cmaes_*_{target}_b{budget}/selection_{budget}_median_val_42.json'))
...
TAVO_SELECTION_PATHS = {'NACT': [SELECTION_ROOT / 'mamamia_clean' / 'outputs' / 'meta' / 'cmaes_nnunet500' / 'selection_250_median_val_42.json', SELECTION_ROOT / 'mamamia_clean' / 'outputs' / 'meta' / 'cmaes_20gen' / 'selection_250_median_val_42.json', SELECTION_ROOT / 'mamamia_clean' / 'outputs' / 'meta' / 'cmaes' / 'selection_250_median_val_42.json'], 'ISPY1': [SELECTION_ROOT / 'mamamia_ispy1' / 'outputs' / 'meta' / 'cmaes_nnunet500' / 'selection_250_median_val_42.json', SELECTION_ROOT / 'mamamia_ispy1' / 'outputs' / 'meta' / 'bo_sh_eta3_nnunet500' / 'selection_250_median_val_eta3_42.json'], 'DUKE': [SELECTION_ROOT / 'mamamia_duke' / 'outputs' / 'selections' / 'meta_cmaes_250.json', SELECTION_ROOT / 'mamamia_duke' / 'outputs' / 'selections' / 'meta_bo_sh_eta3_250.json'], 'ISPY2': [SELECTION_ROOT / 'mamamia_ispy2' / 'outputs' / 'selections' / 'meta_cmaes_gen6_250.json', SELECTION_ROOT / 'mamamia_ispy2' / 'outputs' / 'selections' / 'meta_bo_gen7_250.json']}
...
def candidate_paths(target: str, method: str, budget: int) -> list[Path]:
    ...
    if method == 'tavo':
        ...
        candidates.extend([DERIVED_TUMORSEG_SELECTION_ROOT / target / f'{method}_{budget}.json', DERIVED_TUMORSEG_SELECTION_ROOT / target / f'{method}_250.json'])
    if method == 'tavo':
        pass
        ...
        candidates.extend([target_dir / f'{method}_proxy_{budget}.json', target_dir / f'{method}_proxy_250.json', target_dir / f'{method}_{budget}.json', target_dir / f'{method}_250.json'])
    else:
...
def materialize_one(args: argparse.Namespace, target: str, method: str, budget: int) -> dict[str, Any]:
    ...
    for path in candidate_paths(target, method, budget):
        ...
        attempt = {'path': str(path), 'raw_count': len(ordered), 'valid_source_count': len(valid), 'duplicate_count': duplicates, 'skipped_not_source_count': len(skipped_not_source)}
        ...
        if args.dry_run:
            ...
            provenance = {'target': target, 'method': method, 'budget': budget, 'output': str(output_path), 'source_artifact': str(path), 'artifact_method': data.get('method'), 'selected_count': len(selected), 'valid_source_count': len(valid), 'raw_count': len(ordered), 'duplicate_count': duplicates, 'skipped_not_source_count': len(skipped_not_source), 'skipped_not_source_preview': skipped_not_source[:25], 'all_attempts': attempts}
            provenance_path.write_text(json.dumps(provenance, indent=2) + '\n')
            status = 'written'
        return {'target': target, 'method': method, 'budget': budget, 'status': status, 'output': str(output_path), 'source_artifact': str(path), 'selected_count': budget}
    result = {'target': target, 'method': method, 'budget': budget, 'status': 'missing_or_insufficient', 'attempts': attempts}
    if args.strict:
...
def main() -> None:
    ...
    results = [materialize_one(args, target, method, budget) for target in targets for budget in args.budgets for method in methods]
