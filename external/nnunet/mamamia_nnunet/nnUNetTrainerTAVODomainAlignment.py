...
class nnUNetTrainerTAVOAlignmentBase(nnUNetTrainerTAVOSaveEveryEpoch):
    """MAMAMIA domain-alignment base trainer.

    These trainers keep the regular nnU-Net supervised Dice/CE training over
    the existing MAMAMIA train split, then add a target-aware alignment loss
    between source-domain and target-domain cases in the same mini-batch. Case
    domain is inferred from the MAMAMIA case-id prefix.
    """
    ...
    def configure_optimizers(self):
        if self.uses_discriminator:
            self.discriminator = PooledMapDiscriminator(
    ...
    def train_step(self, batch: dict) -> dict:
        ...
        return {
            ...
            "align_loss": align_loss.detach().cpu().numpy(),
            "aligned_batches": np.array(float(source_mask.any() and target_mask.any()), dtype=np.float32),
        }
    ...
    def on_train_epoch_end(self, train_outputs):
        ...
        self.print_to_log_file(
            f"{self.alignment_method}_seg_loss {np.round(seg_loss, decimals=4)} "
            f"{self.alignment_method}_align_loss {np.round(align_loss, decimals=4)} "
            f"aligned_batch_fraction {np.round(aligned, decimals=4)}"
        )
