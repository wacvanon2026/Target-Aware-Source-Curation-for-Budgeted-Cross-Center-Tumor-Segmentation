...
def main() -> None:
    ...
    values = {'TARGET': target, 'EXPERIMENT': exp.key, 'OFFSET': exp.offset, 'SUFFIX': exp.suffix, 'LABEL': exp.label, 'DATASET_ID': dataset_id(target, exp), 'DATASET_NAME': dataset_name(target, exp), 'DATASET_BASENAME': dataset_basename(target, exp), 'EXPERIMENT_BLOCK': exp.block, 'EXPERIMENT_METHOD': exp.method or '', 'EXPERIMENT_BUDGET': exp.budget or ''}
    if args.shell:
