...
TARGET_ROOTS = {'NACT': 'mamamia_clean', 'ISPY1': 'mamamia_ispy1', 'DUKE': 'mamamia_duke', 'ISPY2': 'mamamia_ispy2'}
TARGET_FULL_SOURCE_DATASETS = {'NACT': 'Dataset1303_MAMAMIA_NACT_LODO_SEED42_TAVO_TARGET_FULL_SOURCE_2d_3ch', 'ISPY1': 'Dataset1313_MAMAMIA_ISPY1_LODO_SEED42_TAVO_TARGET_FULL_SOURCE_2d_3ch', 'DUKE': 'Dataset1323_MAMAMIA_DUKE_LODO_SEED42_TAVO_TARGET_FULL_SOURCE_2d_3ch', 'ISPY2': 'Dataset1333_MAMAMIA_ISPY2_LODO_SEED42_TAVO_TARGET_FULL_SOURCE_2d_3ch'}
...
def combine_scores(score_dicts: dict[str, dict[str, float]], weights_vec: np.ndarray) -> dict[str, float]:
...
def select_top_b(score_dicts: dict[str, dict[str, float]], weights_vec: np.ndarray, budget: int) -> tuple[list[str], dict[str, float], dict[str, float]]:
    weights_vec = project_simplex(weights_vec)
    weights = {name: float(weights_vec[i]) for i, name in enumerate(METHODS)}
...
class CachedMamamiaSliceDataset(Dataset):
    _index_cache: dict[tuple[tuple[str, ...], str, int, bool], tuple[Path, list[int], list[bool]]] = {}

    def __init__(self, data_dirs: list[Path], case_ids: list[str], augment: bool=False, min_tumor_pixels: int=100, crop_size: int=512, include_all: bool=False):
        ...
        print(f'Loaded {len(self.slices)} slices from {len(case_ids)} cases ({n_fg} tumor, {n_bg} non-tumor; cached index cases={len(self._index_cache)})')
    ...
    def _index_case(self, case_id: str, min_tumor_pixels: int, include_all: bool) -> tuple[Path, list[int], list[bool]] | None:
        case_dirs: list[Path] = []
        for data_dir in self.data_dirs:
            if (data_dir / f'{case_id}.npy').exists() and (data_dir / f'{case_id}_seg.npy').exists():
    ...
    def __getitem__(self, idx: int) -> dict[str, torch.Tensor | str | int]:
        ...
        return {'image': img_t, 'mask': mask_t, 'case_id': case_id, 'slice_idx': slice_idx}
...
def load_target_arrays(target_root: Path) -> tuple[list[str], np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    ...
    if len(common_ids) != len(emb_ids) or len(common_ids) != len(grad_ids):
        print(f'WARNING: aligning pool to embedding/gradient intersection: embeddings={len(emb_ids)}, gradients={len(grad_ids)}, common={len(common_ids)}')
    if set(query_emb_ids) != set(query_grad_ids):
        print(f'WARNING: query embedding/gradient ID sets differ; embedding_query={len(query_emb_ids)}, gradient_query={len(query_grad_ids)}')
...
def compute_score_dicts(target_root: Path, budget: int, seed: int) -> dict[str, dict[str, float]]:
    ...
    for method_name in METHODS:
        ...
        result = method.select(pool_ids=pool_ids, budget=budget, embeddings=pool_emb, query_embeddings=query_emb, gradients=pool_grad, query_gradients=query_grad)
...
def build_fitness_evaluator(args: argparse.Namespace, target_root: Path):
    ...
    nnunet_preprocessed = args.project_root / 'externals' / 'MAMA-MIA' / 'nnUNet' / 'nnunetv2' / 'nnUNet_preprocessed'
    data_dirs = []
    if args.dataset_name:
        dataset_names = args.dataset_name
    else:
        dataset_names = [TARGET_FULL_SOURCE_DATASETS[args.target]]
        dataset_names.extend((name for target, name in TARGET_FULL_SOURCE_DATASETS.items() if target != args.target))
    ...
    evaluator = evaluator_cls(data_dirs=data_dirs, val_cases=target_val, batch_size=args.batch_size, lr=args.lr, crop_size=args.crop_size, best_k=args.best_k)
    return (evaluator, target_train)

def evaluate_candidate(score_dicts: dict[str, dict[str, float]], evaluator: Any, target_train: list[str], budget: int, weights_vec: np.ndarray, n_steps: int, seeds: list[int]) -> dict[str, Any]:
    ...
    for seed in seeds:
        ...
        per_seed[str(seed)] = {'fitness': float(result['fitness']), 'final_val_dice': result.get('final_val_dice'), 'val_dices': result.get('val_dices', []), 'n_train_slices': result.get('n_train_slices'), 'elapsed_seconds': elapsed}
    fitness = aggregate([v['fitness'] for v in per_seed.values()], 'median')
    return {'weights': weights, 'n_steps': int(n_steps), 'fitness': float(fitness), 'per_seed': per_seed, 'selected_cases': selected, 'selected_scores': {case: combined[case] for case in selected}, 'elapsed_seconds': elapsed_total}
...
def run_cma(args: argparse.Namespace) -> dict[str, Any]:
    ...
    score_summary = {'target': args.target, 'budget': args.budget, 'methods': METHODS, 'n_sources': len(next(iter(score_dicts.values())))}
    ...
    for gen in range(n_gen):
        ...
        gen_summary = {'gen': gen, 'sigma': float(sigma), 'mean_z': mean.tolist(), 'mean_proj': project_simplex(mean).tolist(), 'eigvals': eigvals.tolist(), 'best_fitness_gen': float(parents[0]['fitness']), 'best_fitness_global': float(best_so_far['fitness'] if best_so_far else float('-inf'))}
        generations.append(gen_summary)
        save_json(partial_path, {'config': vars(args), 'methods': METHODS, 'best_so_far': best_so_far, 'generations': generations, 'n_evals': len(all_evals)})
    ...
    result = {'config': vars(args), 'methods': METHODS, 'score_summary': score_summary, 'generations': generations, 'best_so_far': best_so_far, 'top3': refine_records[:3], 'refine_records': refine_records, 'best': best}
    save_json(args.output_dir / 'stageA2_cma.json', result)
    if best is not None:
        save_json(args.output_dir / f'selection_{args.budget}_median_val_{args.seed}.json', {'selected': best['selected_cases'], 'selected_scores': best['selected_scores'], 'method': 'tavo_8d_cmaes_median_val', 'budget': args.budget, 'weights': best['weights'], 'methods': METHODS, 'fitness': best['fitness']})
    return result
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--dataset-name', nargs='+', default=None, help='Optional override for proxy dataset(s). Defaults to the target-specific TARGET_FULL_SOURCE dataset.')
