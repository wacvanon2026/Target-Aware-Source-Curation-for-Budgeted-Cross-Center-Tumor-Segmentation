#!/usr/bin/env python3
"""Prepare or submit MAMAMIA nnU-Net domain-alignment baseline jobs."""
...
METHODS = {
    "dann": {
        ...
        "suffix": "dann",
        "description": "DANN-style gradient-reversal domain classifier on nnU-Net probability maps.",
    },
    "mmd": {
        "trainer": "nnUNetTrainerTAVOMMD",
        "suffix": "mmd",
        "description": "DAN/MMD-style RBF distribution alignment on pooled class probabilities.",
    },
    "dan": {
        "trainer": "nnUNetTrainerTAVOMMD",
        "suffix": "mmd",
        "description": "Alias for the MMD/DAN-style trainer.",
    },
    "advent": {
        "trainer": "nnUNetTrainerTAVOADVENT",
        "suffix": "advent",
        "description": "ADVENT-style adversarial alignment on prediction entropy maps.",
    },
    "seasa": {
        "trainer": "nnUNetTrainerTAVOSEASA",
        "suffix": "seasa",
        "description": "SE-ASA-style entropy and semantic-distribution alignment.",
    },
    "se-asa": {
        "trainer": "nnUNetTrainerTAVOSEASA",
        "suffix": "seasa",
        "description": "Alias for the SE-ASA-style trainer.",
    },
}
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--methods",
        nargs="+",
        default=["dann", "mmd", "advent", "seasa"],
        choices=sorted(METHODS),
        help="Domain-alignment methods to run.",
    )
    parser.add_argument(
        "--targets",
        nargs="+",
        default=list(DOMAINS),
        help="Targets to run: NACT ISPY1 DUKE ISPY2, or all.",
    )
    parser.add_argument(
        "--experiments",
        nargs="+",
        default=["target_full_source"],
        choices=sorted(EXPERIMENTS),
        help=(
            "Existing MAMAMIA dataset rows to train on. target_full_source is "
            "the intended default because alignment needs source and target cases."
        ),
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Maximum jobs to submit in this invocation; 0 means no limit.",
    )
...
def export_string(args: argparse.Namespace, method: str) -> str:
    ...
    env = {
        ...
        "OUTPUT_SUFFIX": f"_{spec['suffix']}",
        "ADAPT_METHOD": spec["suffix"],
        "ADAPT_LAMBDA": str(args.lambda_),
        "ADAPT_DISC_HIDDEN": str(args.disc_hidden),
        "FORCE": "1" if args.force else "0",
        "AUTO_BUILD_DATASET": "1" if args.auto_build else "0",
        "CLEAN_PREPROCESSED_ON_SUCCESS": "0",
        "CLEAN_PREPROCESSED_ON_ERROR": "0",
    }
        ...
        for experiment in args.experiments:
            for raw_method in args.methods:
                ...
                cmd = [
                    ...
                    f"--job-name={job_name}",
                    f"--export={export_string(args, raw_method)}",
                    str(script),
                    target,
                    experiment,
                ]
