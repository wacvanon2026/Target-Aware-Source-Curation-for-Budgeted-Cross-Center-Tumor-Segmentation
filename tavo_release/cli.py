...
def cmd_smoke(args):
    ...
    result = {'workdir': str(base), 'mamamia_targets': sorted(mamamia_summary), 'officehome': office_summary, 'nnunet_raw_exists': raw.exists(), 'mamamia_train_command': mamamia_cmd, 'officehome_dry_run_command': office_cmd, 'brats_dry_run_command': brats_cmd, 'selected': selected, 'best_score': best['score'], 'plan_steps': plan['steps'], 'forbidden_hits': hits, 'large_files': large}
    if hits or large:
        raise SystemExit(json.dumps(result, indent=2))
    return result
...
def main(argv: list[str] | None=None) -> int:
    ...
    result = {'split': cmd_split, 'download': cmd_download, 'select': cmd_select, 'search': cmd_search, 'tavo-command': cmd_tavo_command, 'selection-route': cmd_selection_route, 'route-inventory': cmd_route_inventory, 'route-audit': cmd_route_audit, 'command': cmd_command, 'da-config': cmd_da_config, 'da-command': cmd_da_command, 'collect': cmd_collect, 'officehome-config': cmd_officehome_config, 'plan': cmd_plan, 'plan-audit': cmd_plan_audit, 'docs-audit': cmd_docs_audit, 'repro-smoke': cmd_repro_smoke, 'matrix': cmd_matrix, 'pathway-audit': cmd_pathway_audit, 'smoke': cmd_smoke, 'check': cmd_check}[args.cmd](args)
