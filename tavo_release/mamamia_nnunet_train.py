...
TRAINER_FILES = ('nnUNetTrainerTAVOSaveEveryEpoch.py', 'nnUNetTrainerTAVODomainAlignment.py')
...
def install_trainers() -> list[Path]:
    ...
    for name in TRAINER_FILES:
        ...
        try:
            ...
            raise PermissionError(f'Cannot install {src.name} into {dst_dir}. Use a writable Python environment or copy the files manually.') from exc
    return copied
