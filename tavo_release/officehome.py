...
def build_splits(data_root: str | Path, output_root: str | Path, target_domain: str, seed: int=42, source_ratios=(9, 1), target_ratios=(7, 1, 2)) -> dict[str, int]:
    ...
    summary = {'source_train': len(source_train), 'source_val': len(source_val), 'target_train': len(target_train), 'target_val': len(target_val), 'target_test': len(target_test)}
...
def build_config(output: str | Path, split_dir: str | Path, output_dir: str | Path, backbone: str='resnet50', epochs: int=30, batch_size: int=64) -> Path:
    cfg = {'dataset': 'officehome', 'splits': {'source_train': str(Path(split_dir) / 'source_train.txt'), 'source_val': str(Path(split_dir) / 'source_val.txt'), 'target_train': str(Path(split_dir) / 'target_train.txt'), 'target_val': str(Path(split_dir) / 'target_val.txt'), 'target_test': str(Path(split_dir) / 'target_test.txt')}, 'model': {'backbone': backbone}, 'training': {'epochs': epochs, 'batch_size': batch_size, 'output_dir': str(Path(output_dir))}}
